...
async function SidebarField({ props }: { props: LayoutParams }) {
  ...
  return (
    <div className="flex flex-col flex-1 gap-2 px-2 py-2 overflow-auto rounded-scrollbar dark:hover:rounded-scrollbar-thumb-neutral-600 hover:rounded-scrollbar-thumb-neutral-400 dark:rounded-scrollbar-track-neutral-800 rounded-scrollbar-track-neutral-200">
      {chapters.map((s, i) => (<SidebarLink href={`/${i+1}/1`} key={i} className={`flex px-4 py-2 rounded-md ${Number.parseInt(params.chapter) == i+1? 'dark:hover:bg-white/20 hover:bg-black/20 dark: bg-black/15': 'dark:hover:bg-white/10 hover:bg-black/10'} text-start`}>{s}</SidebarLink>))}
    </div>
  )
}
...
async function ChapterDetails({ props }: { props: LayoutParams }) {
	...
	return (
		<div className="sm:flex hidden flex-col pb-6">
			...
			<div hidden={chapter.introduction == undefined} className="text-justify mt-8">{chapter.introduction}</div>
...
function ChapterDetailsSkel() {
	return (
		<div className="sm:flex hidden flex-col pb-6 animate-pulse">
			<div className="self-center mb-2"><div className="dark:bg-white/10 bg-black/10 h-9 w-72 rounded-lg"/></div>
			<div className="self-center"><div className="dark:bg-white/10 bg-black/10 h-5 w-64 rounded-lg"/></div>
		</div>
	)
}
...
export default function Layout( props: LayoutParams ) {
  return <AppLayout 
		...
		chatClient={
			<div className="flex flex-col flex-1 overflow-y-auto rounded-scrollbar dark:hover:rounded-scrollbar-thumb-neutral-700 hover:rounded-scrollbar-thumb-neutral-400 dark:rounded-scrollbar-track-neutral-900 rounded-scrollbar-track-white">
